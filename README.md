...
Once you have the preprocessed images, they are now ready for segmentation. Make sure `membrane_submitjob.sh` and `nucleus_submitjob.sh` are modified such that the variable `DATA_PATH` is the path where the code from this repo is located in your Longleaf directory and in the last line `/work/users/{o}/{n}/{onyen}` is changed so that `{o}` is the first letter of your onyen, `{n}` is the second letter, and `{onyen}` is your full onyen. To segment, there are only 2 attributes that needs to be changed in `submit_all_jobs.sh`: the variable `IMG_PATH`, which is the path to the images, and the smallest and largest movie numbers to segment in `{<min>..<max>}` in line 5. Once that is done, you are ready to segment the cell membrane and nucleus by typing: 
